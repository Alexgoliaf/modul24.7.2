...
class PetFriends:
    ...
    def get_list_of_pets(self, auth_key:json, filter:str='my_pets')->json:
        """Метод делает запрос к APIсервера и возвращает статус запроса и результат в формате JSON
        со списком найденных питомцев, совпадавших с фильтром, на данный момент фильтр может иметь либо пустое
    ...
    def post_add_photo(self,auth_key:json,pet_id:str,pet_photo:str)->json:
        """Метод добавляет только фото по ийди питомца"""
        headers={'auth_key':auth_key['key']}
        file = {'pet_photo': (pet_photo, open(pet_photo, 'rb'), 'image/jpeg')}
